...
for i in range(num_episodes_to_save):
    ...
    while not (terminated or truncated):
        # اضافه کردن فریم فعلی به لیست
        frames.append(env.render())
        
        # انتخاب بهترین اکشن بر اساس جدول Q
        action = np.argmax(Q[observation])
        observation, reward, terminated, truncated, info = env.step(action)

# ذخیره فریم‌ها به صورت فایل GIF
imageio.mimsave('1- FrozenLake__deterministic.gif', frames, fps=10)
# imageio.mimsave('2- FrozenLake__stochastic.gif', frames, fps=10)
print("فایل GIF با موفقیت ذخیره شد!")
